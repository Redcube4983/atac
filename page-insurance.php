<?php
/*
Template Name: insurance
*/
?>
<?php get_header(); ?>
<main>
  <section id="l-internalTop">
    <div class="l-internalTop__ornamentLeft">
      <picture>
        <source srcset="<?php echo get_template_directory_uri(); ?>/images/insurance/ornament.webp" type="image/webp">
        <img src="<?php echo get_template_directory_uri(); ?>/images/insurance/ornament.png" alt="">
      </picture>
    </div>
    <h1 class="l-internalTop__title">
      <div class="l-internalTop__text">
        <picture>
          <source srcset="<?php echo get_template_directory_uri(); ?>/images/insurance/pageTitle.webp" type="image/webp">
          <img src="<?php echo get_template_directory_uri(); ?>/images/insurance/pageTitle.png" alt="保険のご紹介">
        </picture>
      </div>
      <div class="l-internalTop__underLine">
        <picture>
          <source srcset="<?php echo get_template_directory_uri(); ?>/images/insurance/ornamentLine.webp" type="image/webp">
          <img src="<?php echo get_template_directory_uri(); ?>/images/insurance/ornamentLine.png" alt="">
        </picture>
      </div>
      <div class="l-internalTop__illust">
        <picture>
          <source srcset="<?php echo get_template_directory_uri(); ?>/images/common/illust.webp" type="image/webp">
          <img src="<?php echo get_template_directory_uri(); ?>/images/common/illust.png" alt="">
        </picture>
      </div>
    </h1>
  </section>

  <section id="p-aboutUsContents">
    <div class="p-aboutUsContents p-aboutUsContents__insuranceArea">
      <div class="p-aboutUsContents__insuranceBox">
        <div class="p-aboutUsContents__insuranceBox-title">
            <h2>生命保険</h2>
        </div>
        <div class="p-aboutUsContents__insuranceBox-textbox">
            <div class="p-aboutUsContents__insuranceBox-lead">
                <b>万一の時、将来のために</b>
            </div>
            <ul class="p-aboutUsContents__insuranceBox-kinds">
                <li>&#9632;&amp;LIFE 新総合収入保障</li>
                <li>&#9632;&amp;LIFE 医療保険Ａエースセレクト</li>
                <li>&#9632;&amp;LIFE 逓減定期保険</li>
                <li>&#9632;&amp;LIFE こども保険</li>
                <li>&#9632;&amp;LIFE 終身保険（低解約返戻金型）</li>
                <li>&#9632;&amp;LIFE 新収入保障</li>
                <li>&#9632;&amp;LIFE ガン保険Ｓスマートセレクト</li>
                <li>&#9632;&amp;LIFE 個人年金保険</li>
                <li>&#9632;&amp;特定疾病保障終身保険</li>
            </ul>
        </div>
      </div>

      <div class="p-aboutUsContents__insuranceBox">
        <div class="p-aboutUsContents__insuranceBox-title">
            <h2>自動車保険</h2>
        </div>
        <div class="p-aboutUsContents__insuranceBox-textbox">
            <div class="p-aboutUsContents__insuranceBox-lead">
                <b>カーライフに安心を</b>
            </div>
            <ul class="p-aboutUsContents__insuranceBox-kinds">
                <li>&#9632;GK クルマの保険（家庭用自動車総合保険）</li>
                <li>&#9632;はじめての自動車保険（個人用自動車保険）</li>
                <li>&#9632;自賠責保険</li>
            </ul>
            <div class="p-aboutUsContents__carImg">
                <a href="https://www.btoc.ms-ins.com/qsc/QSC00000">
                    <picture>
                        <source srcset="<?php echo get_template_directory_uri(); ?>/images/insurance/car.webp" type="image/webp">
                        <img src="<?php echo get_template_directory_uri(); ?>/images/insurance/car.png" alt="">
                    </picture>
                </a>
            </div>
        </div>
      </div>

      <div class="p-aboutUsContents__insuranceBox">
        <div class="p-aboutUsContents__insuranceBox-title">
            <h2>火災保険</h2>
        </div>
        <div class="p-aboutUsContents__insuranceBox-textbox">
            <div class="p-aboutUsContents__insuranceBox-lead">
                <b>快適で安心な暮らしのために</b>
            </div>
            <ul class="p-aboutUsContents__insuranceBox-kinds">
                <li>&#9632;GK すまいの保険（家庭用火災保険）</li>
                <li>&#9632;リビングＦＩＴ（賃貸住宅居住者総合保険）</li>
                <li>&#9632;地震保険</li>
            </ul>
        </div>
      </div>

      <div class="p-aboutUsContents__insuranceBox">
        <div class="p-aboutUsContents__insuranceBox-title">
            <h2>ケガの保険</h2>
        </div>
        <div class="p-aboutUsContents__insuranceBox-textbox">
            <div class="p-aboutUsContents__insuranceBox-lead">
                <b>突然の事故によるケガに</b>
            </div>
            <ul class="p-aboutUsContents__insuranceBox-kinds">
                <li>&#9632;GKケガの保険（パーソナル総合傷害保険）</li>
                <li>&#9632;晴れやか世代（特定傷害保険）</li>
            </ul>
        </div>
      </div>

      <div class="p-aboutUsContents__insuranceBox">
        <div class="p-aboutUsContents__insuranceBox-title">
            <h2>旅行の保険</h2>
        </div>
        <div class="p-aboutUsContents__insuranceBox-textbox">
            <div class="p-aboutUsContents__insuranceBox-lead">
                <b>旅先での突然のトラブルに</b>
            </div>
            <ul class="p-aboutUsContents__insuranceBox-kinds">
                <li>&#9632;海外旅行保険</li>
                <li>&#9632;国内旅行傷害保険</li>
            </ul>
        </div>
      </div>

      <div class="p-aboutUsContents__insuranceBox">
        <div class="p-aboutUsContents__insuranceBox-title">
            <h2>レジャーの保険</h2>
        </div>
        <div class="p-aboutUsContents__insuranceBox-textbox">
            <div class="p-aboutUsContents__insuranceBox-lead">
                <b>レジャーを安心して楽しむために</b>
            </div>
            <ul class="p-aboutUsContents__insuranceBox-kinds">
                <li>&#9632;ゴルファー保険</li>
                <li>&#9632;レクレーション傷害補償プラン</li>
            </ul>
        </div>
      </div>

      <div class="p-aboutUsContents__insuranceBox">
        <div class="p-aboutUsContents__insuranceBox-title">
            <h2>その他の保険</h2>
        </div>
        <div class="p-aboutUsContents__insuranceBox-textbox">
            <div class="p-aboutUsContents__insuranceBox-products">
                <a href="https://www.ms-ins.com/business/">
                <span>企業向け商品・サービス</span>
                </a>
            </div>
            <div class="p-aboutUsContents__bicycleImg">
                <a href="https://net2.ms-ins.com/servlet/scy/SCY_UII_CYS0010?free_area1=&free_area2=">
                    <picture>
                        <source srcset="<?php echo get_template_directory_uri(); ?>/images/insurance/bicycle.webp" type="image/webp">
                        <img src="<?php echo get_template_directory_uri(); ?>/images/insurance/bicycle.png" alt="">
                    </picture>
                </a>
            </div>
        </div>
      </div>
    </div>
  </section>
</main>
<?php get_footer(); ?>